...
Implement the [product requirements](../prd/show-learned-percentage-during-study.md). Show the active collection's learned percentage during both Flashcard and Quiz study as a small, muted “25% Learned” label above the current card or question. Update it immediately after a successful save, including while Quiz Answer Feedback remains visible.
...
- [x] The label uses the format “25% Learned”, appears above the current Flashcard or Quiz Question, and scrolls normally with the content. It is readable but visually secondary on phone and desktop.
